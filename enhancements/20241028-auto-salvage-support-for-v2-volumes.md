...
When `EngineCreate()` is called with `salvage_requested` set to `true`, the SPDK server retrieves `Replica` from SPDK server cache. All replicas lvol head size are then sorted. Replicas with a head size different from the largest size in the sorted list are marked as `ERR`.
